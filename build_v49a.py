#!/usr/bin/env python3
"""
build_v49a.py — Build V49a_Swing_Trade_Tuned.ipynb (V49a code)

Writes all V49a code directly into V49a_Swing_Trade_Tuned.ipynb
so that the notebook can be opened on Colab and run as-is.

V49a changes (on top of V49):
  - Hyperparameter tuning based on V49 initial run analysis.
    V49 saw entropy collapse to floor (0.02) by ep 24 (~13k steps),
    pinning the policy for 98.7% of training. The strong terminal-heavy
    gradients crushed exploration before the agent could discover netting.
  - SAC_ENT_COEF_MIN: 0.02 → 0.08 (4x higher floor, keeps exploring)
  - TERMINAL_REWARD_SCALE: 20.0 → 10.0 (halve gradient pressure)
  - NETTING_BONUS_SCALE: 10.0 → 5.0 (proportional to terminal reduction)
  - GREEN_EPISODE_BONUS: 2.0 → 1.0 (keep reward magnitudes balanced)
  - All V49 features retained: 999-dim obs, 4 new per-runner features,
    urgency penalty, zeroed activity rewards, swing-trade structure.

V49 base:
  - Terminal-heavy reward structure (MTM 0.5x, terminal dominant)
  - 4 new features: session_vwap_ratio, price_percentile, position_age,
    favourite_delta
  - 999-dim observation space (41 features/runner × 24 + 14 + 1)
  - Netting bonus, urgency penalty, zeroed activity rewards
  - All V48 features: entropy floor, stale penalty, temporal deltas,
    TD target clipping, SAC metrics, position netting
"""
...
OUTPUT_PATH = "V49a_Swing_Trade_Tuned.ipynb"
...
# ---------------------------------------------------------------------------
cell_2_src = """\
### CELL 3 - CONFIGURATION (V49a - SWING TRADE TUNED) ###

# V49a: Tuned hyperparameters — higher entropy floor, halved reward scales
# 999-dimensional observation space (41 features/runner + 14 global + 1 green-up)

print("=" * 60)
print("V49a Configuration - Swing Trade Tuned")
print("=" * 60)

# ============================================================
# PATHS
# ============================================================
BASE_PATH = '/content/drive/MyDrive/Betfair_RL/V49a_Swing_Trade_Tuned'
...
print(f"\\n  Configuration loaded for V49a (Swing Trade Tuned)")
...
print(f"\\n  V49a Tuning (on top of V49):")
print(f"   Entropy floor: 0.02 -> {SAC_ENT_COEF_MIN} (V49 collapsed at ep 24)")
print(f"   Terminal reward: 20x -> {TERMINAL_REWARD_SCALE}x (halve gradient pressure)")
print(f"   Netting bonus: 10x -> {NETTING_BONUS_SCALE}x (proportional reduction)")
print(f"   Green bonus: 2.0 -> {GREEN_EPISODE_BONUS} (balanced magnitudes)")
print(f"\\n  V49 Retained:")
...
print(f"   Urgency: scale={URGENCY_SCALE}, start={URGENCY_START}")
print(f"   New features: session_vwap_ratio, price_percentile, position_age, favourite_delta")
...
# ---------------------------------------------------------------------------
cell_3_src = r'''### CELL 4 - ENVIRONMENT & TRAINING COMPONENTS (V49a - SWING TRADE TUNED) ###

# V49a - Swing Trade Tuned: higher entropy floor + halved reward scales
# Builds on V49's terminal-heavy structure + 4 new features (999 dims)
# Tuning: ent_floor 0.08, terminal 10x, netting 5x, green bonus 1.0
...
print("=" * 60)
print("Model Version: V49a_Swing_Trade_Tuned")
print("Algorithm: SAC + TD Clip + Entropy Floor(0.08) + Swing-Trade Rewards (999 dims)")
print("=" * 60)
...
# ============================================================
# MARKET MAKING ENVIRONMENT (V49a - SWING TRADE TUNED)
# ============================================================

class MarketMakingEnv(gym.Env):
    """V49a: 999-dim obs with tuned swing-trade rewards + 4 new features."""
    ...
    def __init__(self, race_files, curriculum_tracker=None):
        ...
        print(f"\n[ENV INIT] V49a Swing Trade Tuned ({self.OBS_DIM} dimensions)")
...
def sync_metrics_to_github(training_csv_path=None, validation_csv_path=None, message="Update metrics"):
    ...
    try:
        ...
        if training_csv_path and os.path.exists(training_csv_path):
            dst = os.path.join(repo_dir, 'training_metrics_v49a.csv')
            shutil.copy2(training_csv_path, dst)
            files_copied.append('training_metrics_v49a.csv')
        if validation_csv_path and os.path.exists(validation_csv_path):
            dst = os.path.join(repo_dir, 'validation_metrics_v49a.csv')
            shutil.copy2(validation_csv_path, dst)
            files_copied.append('validation_metrics_v49a.csv')
...
print("\n  V49a Swing Trade Tuned — Environment and callbacks loaded!")
print("  999-dimensional observation space (41/runner + 14 global + 1 green-up)")
print("  V49 features: session_vwap_ratio, price_percentile, position_age, favourite_delta")
print("  V49a reward: MTM(0.5x) + Terminal(10x) + NettingBonus(5x) + Urgency + GreenBonus(1.0)")
print(f"  V49a entropy floor: {SAC_ENT_COEF_MIN} (raised from V49's 0.02)")
...
# ---------------------------------------------------------------------------
cell_4_src = r'''### CELL 5 - SETUP TRAINING + CHECKPOINT RESUME (V49a) ###
...
print("  Training env: NoTradeStreakWrapper -> Monitor -> MarketMakingEnv (999 dims, V49a)")
...
training_callback = TrainingMetricsCallback(
    log_interval=1,
    save_path=f"{BASE_PATH}/training_metrics_v49a.csv",
    curriculum_tracker=curriculum,
)
validation_callback = ValidationCallback(
    val_env=val_env,
    val_interval=50000,
    save_path=f"{BASE_PATH}/validation_metrics_v49a.csv",
    training_csv_path=f"{BASE_PATH}/training_metrics_v49a.csv",
)
...
if checkpoint_path:
    ...
    apply_v49_patches(model)
    TRAINING_STEPS = 1000000  # V49a: 1M steps
    RESUME_MODE = False

print("\n" + "=" * 60)
print("  V49a Setup complete — ready for training!")
print("  (Swing Trade Tuned: ent_floor=0.08, terminal=10x, netting=5x, green=1.0)")
print("=" * 60)
'''
...
print("=" * 60)
if RESUME_MODE:
    print(f"  CONTINUING V49a training from checkpoint")
    print(f"  Training for {TRAINING_STEPS:,} additional steps")
else:
    print(f"  Starting V49a training run (Swing Trade Tuned — ent_floor=0.08, terminal=10x)")
...
print(f"  V49a: Swing trade tuned — ent_floor=0.08, terminal=10x, netting=5x, green=1.0 (999 dims)")
