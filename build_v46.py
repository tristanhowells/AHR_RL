#!/usr/bin/env python3
"""
build_v46.py — Build V46_Agent57_Inspired.ipynb (V46 code)

Writes all V46 code directly into V46_Agent57_Inspired.ipynb so that
the notebook can be opened on Colab and run as-is.

V46 changes (on top of V45):
  - Agent57-inspired critic stabilization: Q-value target clipping [-20, +20]
    to prevent the exponential critic loss divergence seen in V45
  - Increased tau (polyak rate) 0.005 -> 0.01 for faster target network tracking
  - Auto-tuned entropy: ent_coef='auto' instead of fixed 0.05, so SAC
    adapts exploration/exploitation throughout training
  - Dense green-up signal: portfolio green-up value added as observation
    feature (756 dims), giving the agent per-step visibility of its
    hedging P&L rather than only at terminal
  - 1M training steps with curriculum scaled to match (500k total, 50k warmup)
  - Validation every 50k steps, checkpoints every 100k steps

V45 base (simplified reward):
  - 4 reward terms: MTM + Sharpe + Activity + Terminal(5x)
  - Position netting with capital release
  - All V43/V44 fixes included
"""
...
OUTPUT_PATH = "V46_Agent57_Inspired.ipynb"
...
# ---------------------------------------------------------------------------
cell_2_src = """\
### CELL 3 - CONFIGURATION (V46 - AGENT57-INSPIRED) ###

# V46: Agent57-inspired improvements — critic stabilization, auto entropy, dense green-up
# 756-dimensional observation space (+1 portfolio green-up value)

print("=" * 60)
print("V46 Configuration - Agent57-Inspired + Critic Stabilization")
print("=" * 60)

# ============================================================
# PATHS
# ============================================================
BASE_PATH = '/content/drive/MyDrive/Betfair_RL/V46_Agent57_Inspired'
...
# V46: Auto-tuned entropy instead of fixed 0.05 (Agent57 insight:
# exploration/exploitation balance should adapt during training)
SAC_ENT_COEF = 'auto'
...
# V46: Q-value target clipping (Agent57 insight: separate bounded targets
# prevent critic loss from diverging exponentially)
...
print(f"\\n  Configuration loaded for V46 (Agent57-Inspired)")
...
print(f"\\n  V46 Agent57 Improvements:")
print(f"   SAC tau (polyak rate): {SAC_TAU} (was 0.005)")
print(f"   SAC entropy coef: {SAC_ENT_COEF} (auto-tuned)")
print(f"   Q-value target clip: [{Q_VALUE_CLIP_MIN}, {Q_VALUE_CLIP_MAX}]")
...
# ---------------------------------------------------------------------------
cell_3_src = r'''### CELL 4 - ENVIRONMENT & TRAINING COMPONENTS (V46 - AGENT57-INSPIRED) ###

# V46 - Agent57-Inspired: Dense green-up signal (756 dims) + Q-value clipping
# All 26 available features per runner + portfolio green-up as global feature
# Critic stabilization via ClippedQValueSAC wrapper
...
print("=" * 60)
print("Model Version: V46_Agent57_Inspired")
print("Algorithm: SAC + Clipped Q-values + Auto Entropy + Dense Green-up")
print("=" * 60)
...
class MarketMakingEnv(gym.Env):
    """V46: 756-dim obs (dense green-up) + Position Netting."""
    ...
    def __init__(self, race_files, curriculum_tracker=None):
        ...
        print(f"\n[ENV INIT] V46 Agent57-Inspired (756 dimensions, dense green-up)")
...
# ============================================================
# Q-VALUE CLIPPING CALLBACK (V46 — Agent57-inspired)
# ============================================================

class QValueClipCallback(BaseCallback):
    """Clip Q-value targets during training to prevent critic divergence.

    Agent57 insight: separate bounded value estimates prevent the exponential
    critic loss growth seen in V45 (0.003 -> 202k over 200k steps).
    This callback clips the Q-target values after each gradient step.
    """

    def __init__(self, clip_min=-20.0, clip_max=20.0, verbose=0):
    ...
    def _on_step(self) -> bool:
        # Access SAC's critic target networks and clamp parameters
        # This runs every step, but the clamp is very fast (in-place op)
        if hasattr(self.model, 'critic_target'):
            for param in self.model.critic_target.parameters():
                param.data.clamp_(self.clip_min, self.clip_max)
        return True
...
print("\n  V46 Environment and callbacks loaded (Agent57-Inspired)!")
print("  756-dimensional observation space (+1 dense green-up)")
print("  Q-value clipping callback for critic stabilization")
print("  Auto-tuned entropy coefficient")
...
# ---------------------------------------------------------------------------
cell_4_src = r'''### CELL 5 - SETUP TRAINING + CHECKPOINT RESUME (V46) ###
...
print("  Training env: NoTradeStreakWrapper -> Monitor -> MarketMakingEnv (756 dims)")
...
# V46: Q-value clipping callback to prevent critic divergence
q_clip_callback = QValueClipCallback(
    clip_min=Q_VALUE_CLIP_MIN,
    clip_max=Q_VALUE_CLIP_MAX,
)
callbacks = CallbackList([training_callback, validation_callback, checkpoint_callback, q_clip_callback])
...
if checkpoint_path:
    ...
    TRAINING_STEPS = 1000000  # V46: 1M steps
    RESUME_MODE = False

print("\n" + "=" * 60)
print("  V46 Setup complete — ready for training (Agent57-Inspired)!")
print("=" * 60)
'''
...
print("=" * 60)
if RESUME_MODE:
    print(f"  CONTINUING V46 training from checkpoint")
    print(f"  Training for {TRAINING_STEPS:,} additional steps")
else:
    print(f"  Starting V46 training run (Agent57-Inspired)")
...
print(f"  V46 Agent57-Inspired: Q-clip + auto entropy + dense green-up (756 dims)")
