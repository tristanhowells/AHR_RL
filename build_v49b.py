#!/usr/bin/env python3
"""
build_v49b.py — Build V49b_Primacy_Reset.ipynb (V49b code)

Writes all V49b code directly into V49b_Primacy_Reset.ipynb
so that the notebook can be opened on Colab and run as-is.

V49b changes (on top of V49a):
  - Implements Nikishin et al. (2022) periodic resets to combat primacy bias.
    V49a showed actor_loss frozen at -22.3 to -22.5 for 27k+ steps —
    textbook primacy bias where early overfitting prevents later learning.
    Netting bonus never activated (0.000 across all 35k steps).
  - NEW: PeriodicResetCallback — resets actor, critic, critic_target, and
    log_ent_coef every 200k env steps. Replay buffer preserved.
  - NEW: Replay buffer saved at each checkpoint (+ loaded on resume) so
    restarts maintain training continuity.
  - SAC_GRADIENT_STEPS: 1 → 4 (replay ratio 4x, lets reset networks
    catch up quickly from preserved buffer data).
  - All V49a values retained: ent_floor=0.08, terminal=10x, netting=5x,
    green=1.0, 999-dim obs, 4 new features, urgency, swing-trade structure.

V49a base:
  - SAC_ENT_COEF_MIN = 0.08 (entropy floor)
  - TERMINAL_REWARD_SCALE = 10.0, NETTING_BONUS_SCALE = 5.0
  - GREEN_EPISODE_BONUS = 1.0
  - 999-dim observation space (41 features/runner × 24 + 14 + 1)
  - All V49/V48 features: stale penalty, temporal deltas, TD clipping
"""
...
OUTPUT_PATH = "V49b_Primacy_Reset.ipynb"
...
# ---------------------------------------------------------------------------
cell_2_src = """\
### CELL 3 - CONFIGURATION (V49b - PRIMACY RESET) ###

# V49b: Nikishin periodic resets + replay buffer checkpoints + replay ratio 4
# 999-dimensional observation space (41 features/runner + 14 global + 1 green-up)

print("=" * 60)
print("V49b Configuration - Primacy Reset")
print("=" * 60)

# ============================================================
# PATHS
# ============================================================
BASE_PATH = '/content/drive/MyDrive/Betfair_RL/V49b_Primacy_Reset'
...
SAC_GAMMA = 0.99
SAC_TRAIN_FREQ = 1
# V49b: Increased from 1 — higher replay ratio lets reset networks catch up from buffer
...
SAC_ENT_COEF = 'auto'
# V49a: Raised from 0.02 — V49 hit floor at ep 24 (13k steps), killing exploration
SAC_ENT_COEF_MIN = 0.08
SAC_ENT_COEF_MAX = 0.5

# V49b: Nikishin periodic reset parameters
RESET_FREQ = 200000  # Reset networks every 200k env steps
RESET_ENT_COEF = True  # Also reset log_ent_coef to initial value at each reset
...
# V49a: Halved from 20.0 — strong gradients crushed entropy before agent discovered netting
TERMINAL_REWARD_SCALE = 10.0

# V49a: Halved from 10.0 — proportional to terminal scale reduction
NETTING_BONUS_SCALE = 5.0
...
# V49a: Halved from 2.0 — keep reward magnitudes balanced with lower terminal scale
GREEN_EPISODE_BONUS = 1.0
...
print(f"\\n  Configuration loaded for V49b (Primacy Reset)")
...
print(f"\\n  V49b Changes (on top of V49a):")
print(f"   Periodic resets: every {RESET_FREQ:,} steps (Nikishin et al. 2022)")
print(f"   Reset targets: actor + critic + critic_target + log_ent_coef")
print(f"   Replay buffer: saved at checkpoints, loaded on resume")
print(f"   Replay ratio: gradient_steps={SAC_GRADIENT_STEPS} (up from 1)")
print(f"\\n  V49a Retained:")
print(f"   Entropy floor: {SAC_ENT_COEF_MIN} | Terminal: {TERMINAL_REWARD_SCALE}x | Netting: {NETTING_BONUS_SCALE}x | Green: {GREEN_EPISODE_BONUS}")
print(f"   Reward: MTM={MTM_REWARD_SCALE}x + Terminal={TERMINAL_REWARD_SCALE}x + NettingBonus={NETTING_BONUS_SCALE}x")
...
print(f"   Stale-market penalty: {STALE_TRADE_PENALTY} | TD clip: [{Q_VALUE_CLIP_MIN}, {Q_VALUE_CLIP_MAX}]")
print(f"\\n  Reward: MTM(0.5x) + Sharpe + Terminal(10x) + NettingBonus(5x) + Urgency + StalePenalty + GreenBonus(1.0)")
print("=" * 60)"""

# ---------------------------------------------------------------------------
# CELL 3 — Environment & Training Components  (ALL FIXES)
# ---------------------------------------------------------------------------
cell_3_src = r'''### CELL 4 - ENVIRONMENT & TRAINING COMPONENTS (V49b - PRIMACY RESET) ###

# V49b - Primacy Reset: Nikishin periodic resets + replay buffer checkpoints
# Builds on V49a's tuned rewards + V49's 4 new features (999 dims)
# New: periodic network resets every 200k steps, replay ratio 4x, buffer saved at checkpoints
...
print("=" * 60)
print("Model Version: V49b_Primacy_Reset")
print("Algorithm: SAC + TD Clip + Entropy Floor(0.08) + Periodic Resets + Replay Ratio 4 (999 dims)")
print("=" * 60)
...
# ============================================================
# MARKET MAKING ENVIRONMENT (V49b - PRIMACY RESET)
# ============================================================

class MarketMakingEnv(gym.Env):
    """V49b: 999-dim obs with periodic resets + tuned swing-trade rewards."""
    ...
    def __init__(self, race_files, curriculum_tracker=None):
        ...
        print(f"\n[ENV INIT] V49b Primacy Reset ({self.OBS_DIM} dimensions)")
...
# ============================================================
# CHECKPOINT CALLBACK (V49b: saves replay buffer too)
# ============================================================
...
class CheckpointCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.num_timesteps % self.save_freq == 0 and self.num_timesteps > 0:
            ...
            # V49b: Save replay buffer alongside model checkpoint
            replay_path = f"{self.save_path}/replay_{self.num_timesteps}"
            self.model.save_replay_buffer(replay_path)
            buf_size = self.model.replay_buffer.size()
            print(f"  Model + replay buffer saved: {path} ({buf_size:,} transitions)")
        return True


# ============================================================
# PERIODIC RESET CALLBACK (V49b: Nikishin et al. 2022)
# ============================================================

class PeriodicResetCallback(BaseCallback):
    """Periodically reset network weights to combat primacy bias.

    Resets actor, critic, critic_target, and log_ent_coef every
    reset_freq env steps. Replay buffer is preserved so the fresh
    network can learn from all accumulated experience.

    Reference: Nikishin et al. "The Primacy Bias in Deep RL" (ICML 2022)
    """
    def __init__(self, reset_freq=200000, reset_ent_coef=True, initial_ent_coef=0.5):
        super().__init__()
        self.reset_freq = reset_freq
        self.reset_ent_coef = reset_ent_coef
        self.initial_ent_coef = initial_ent_coef
        self.last_reset = 0
        self.reset_count = 0

    def _on_step(self) -> bool:
        if (self.num_timesteps - self.last_reset) >= self.reset_freq and self.num_timesteps > 0:
            self._reset_networks()
            self.last_reset = self.num_timesteps
            self.reset_count += 1
        return True
    ...
    def _reset_networks(self):
        ...
        # Reset actor network (policy)
        for module in model.actor.latent_pi.modules():
            if hasattr(module, 'reset_parameters'):
                module.reset_parameters()
        # Reset actor output (mu and log_std)
        if hasattr(model.actor, 'mu'):
            model.actor.mu.reset_parameters()
        ...
        if self.reset_ent_coef and hasattr(model, 'log_ent_coef'):
            with torch.no_grad():
                model.log_ent_coef.fill_(np.log(self.initial_ent_coef))

        print(f"\n  {'='*50}")
        print(f"  [RESET #{self.reset_count + 1}] Periodic network reset at step {self.num_timesteps:,}")
        print(f"  [RESET] Actor + Critic + Critic Target re-initialized")
        if self.reset_ent_coef:
            print(f"  [RESET] log_ent_coef reset to ln({self.initial_ent_coef}) = {np.log(self.initial_ent_coef):.3f}")
        print(f"  [RESET] Replay buffer PRESERVED ({buf_size:,} transitions)")
        print(f"  {'='*50}\n")
...
def sync_metrics_to_github(training_csv_path=None, validation_csv_path=None, message="Update metrics"):
    ...
    try:
        ...
        if training_csv_path and os.path.exists(training_csv_path):
            dst = os.path.join(repo_dir, 'training_metrics_v49b.csv')
            shutil.copy2(training_csv_path, dst)
            files_copied.append('training_metrics_v49b.csv')
        if validation_csv_path and os.path.exists(validation_csv_path):
            dst = os.path.join(repo_dir, 'validation_metrics_v49b.csv')
            shutil.copy2(validation_csv_path, dst)
            files_copied.append('validation_metrics_v49b.csv')
...
print("\n  V49b Primacy Reset — Environment and callbacks loaded!")
print("  999-dimensional observation space (41/runner + 14 global + 1 green-up)")
print("  V49b: Nikishin periodic resets every 200k steps (actor+critic+ent_coef)")
print("  V49b: Replay buffer saved at checkpoints, replay ratio 4x")
print("  V49a reward: MTM(0.5x) + Terminal(10x) + NettingBonus(5x) + Urgency + GreenBonus(1.0)")
print(f"  Entropy floor: {SAC_ENT_COEF_MIN} | Gradient steps: {SAC_GRADIENT_STEPS}")
...
# ---------------------------------------------------------------------------
cell_4_src = r'''### CELL 5 - SETUP TRAINING + CHECKPOINT RESUME (V49b) ###
...
print("  Training env: NoTradeStreakWrapper -> Monitor -> MarketMakingEnv (999 dims, V49b)")
...
training_callback = TrainingMetricsCallback(
    log_interval=1,
    save_path=f"{BASE_PATH}/training_metrics_v49b.csv",
    curriculum_tracker=curriculum,
)
validation_callback = ValidationCallback(
    val_env=val_env,
    val_interval=50000,
    save_path=f"{BASE_PATH}/validation_metrics_v49b.csv",
    training_csv_path=f"{BASE_PATH}/training_metrics_v49b.csv",
)
checkpoint_callback = CheckpointCallback(save_freq=100000, save_path=BASE_PATH)
# V49b: Nikishin periodic reset callback
reset_callback = PeriodicResetCallback(
    reset_freq=RESET_FREQ,
    reset_ent_coef=RESET_ENT_COEF,
    initial_ent_coef=0.5,  # Reset to SAC default, will decay naturally
)
callbacks = CallbackList([training_callback, validation_callback, checkpoint_callback, reset_callback])
...
if checkpoint_path:
    ...
    # V49b: Load replay buffer if saved alongside checkpoint
    if resume_step > 0:
        replay_path = os.path.join(BASE_PATH, f"replay_{resume_step}.pkl")
    else:
        replay_path = os.path.join(BASE_PATH, "replay_final.pkl")
    if os.path.exists(replay_path):
        model.load_replay_buffer(replay_path)
        print(f"  Replay buffer loaded: {os.path.basename(replay_path)} ({model.replay_buffer.size():,} transitions)")
    else:
        print(f"  No replay buffer found at {os.path.basename(replay_path)} — starting with empty buffer")

    # Advance curriculum to resume point (already graduated if past total)
    if resume_step > 0:
        curriculum.current_step = resume_step
        # V49b: Set reset callback's last_reset to align with resume step
        reset_callback.last_reset = (resume_step // RESET_FREQ) * RESET_FREQ
    else:
    ...
    print(f"  Periodic resets: every {RESET_FREQ:,} steps (next at ~{reset_callback.last_reset + RESET_FREQ:,})")
    ...
    apply_v49_patches(model)
    TRAINING_STEPS = 1000000  # V49b: 1M steps
    RESUME_MODE = False

print("\n" + "=" * 60)
print("  V49b Setup complete — ready for training!")
print("  (Primacy Reset: resets every 200k steps, replay ratio 4, ent_floor=0.08)")
print("=" * 60)
'''
...
print("=" * 60)
if RESUME_MODE:
    print(f"  CONTINUING V49b training from checkpoint")
    print(f"  Training for {TRAINING_STEPS:,} additional steps")
else:
    print(f"  Starting V49b training run (Primacy Reset — resets@200k, replay_ratio=4, ent_floor=0.08)")
...
# V49b: Save final replay buffer for potential resume
model.save_replay_buffer(f"{BASE_PATH}/replay_final")
print(f"\n{'='*60}")
print(f"  Training complete!")
print(f"  Final model saved: {final_model_path}")
print(f"  Final replay buffer saved: {BASE_PATH}/replay_final.pkl")
print(f"  Metrics saved to: {BASE_PATH}/")
print(f"  Periodic resets performed: {reset_callback.reset_count}")
print(f"{'='*60}")
'''
...
print(f"  V49b: Primacy Reset — resets@200k, replay_ratio=4, ent_floor=0.08, buffer saved (999 dims)")
