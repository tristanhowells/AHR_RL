#!/usr/bin/env python3
"""
build_v48.py — Build V48_Entropy_Floor_Stale_Penalty.ipynb (V48 code)

Writes all V48 code directly into V48_Entropy_Floor_Stale_Penalty.ipynb
so that the notebook can be opened on Colab and run as-is.

V48 changes (on top of V47):
  - Entropy floor: ent_coef clamped to [0.02, 0.5] instead of [0, 0.5].
    V47 saw ent_coef collapse to 0.002 by episode 400 (~220k steps),
    killing exploration with 90% of training remaining. The floor keeps
    the policy stochastic enough to discover better strategies.
  - Stale-market penalty: instead of silently skipping stale trades,
    the agent now receives a negative reward (-0.005) per attempted
    stale-market trade. This gives the agent a gradient signal to stop
    trying to trade in stale markets (2.58M violations in V47 run).
  - Green episode bonus: a positive terminal reward (+0.5 scaled)
    when the episode ends profitably (PnL > 0). V47's reward was
    symmetric — losses and gains treated equally — giving no extra
...
OUTPUT_PATH = "V48_Entropy_Floor_Stale_Penalty.ipynb"
...
# PATHS
# ============================================================
BASE_PATH = '/content/drive/MyDrive/Betfair_RL/V48_Entropy_Floor_Stale_Penalty'
...
# V48: Green episode bonus — extra terminal reward for profitable episodes
# V47's symmetric reward gave no extra incentive to be profitable.
# This bonus reinforces episodes where the agent makes money.
GREEN_EPISODE_BONUS = 0.5
...
print(f"\\n  Configuration loaded for V48 (Entropy Floor + Stale Penalty + Green Bonus)")
...
print(f"   Green episode bonus: +{GREEN_EPISODE_BONUS} terminal reward for profitable episodes")
print(f"\\n  V47 Retained:")
...
# ---------------------------------------------------------------------------
cell_3_src = r'''### CELL 4 - ENVIRONMENT & TRAINING COMPONENTS (V48 - ENTROPY FLOOR + STALE PENALTY + GREEN BONUS) ###

# V48 - Entropy Floor + Stale Penalty + Green Bonus
# Builds on V47's TD target clipping + SAC metrics logging
# New: entropy floor (0.02), stale-market penalty, green episode bonus
...
print("=" * 60)
print("Model Version: V48_Entropy_Floor_Stale_Penalty")
print("Algorithm: SAC + TD Clip + Entropy Floor + Stale Penalty + Green Bonus")
print("=" * 60)


# ============================================================
# CLIPPED TD TARGET SAC (V47/V48 — TD clip + entropy floor)
# ============================================================
...
def apply_v48_patches(model):
    """Apply V47 critic target clipping + V48 entropy floor/cap."""
    ...
    if hasattr(model, 'log_ent_coef') and model.log_ent_coef is not None:
        ...
        print(f"  [V48] Current ent_coef: {current_ent:.4f} (floor: {model.ent_coef_min}, cap: {model.ent_coef_max})")
        clamped = False
        if current_ent > model.ent_coef_max:
            max_log_ent = np.log(model.ent_coef_max)
            with torch.no_grad():
                model.log_ent_coef.data.fill_(max_log_ent)
            print(f"  [V48] Clamped ent_coef from {current_ent:.4f} down to {model.ent_coef_max}")
            clamped = True
        if current_ent < model.ent_coef_min:
            min_log_ent = np.log(model.ent_coef_min)
            with torch.no_grad():
                model.log_ent_coef.data.fill_(min_log_ent)
            print(f"  [V48] Raised ent_coef from {current_ent:.4f} up to {model.ent_coef_min}")
            clamped = True
        if not clamped:
            print(f"  [V48] ent_coef {current_ent:.4f} within bounds — no adjustment needed")
...
# ============================================================
# MARKET MAKING ENVIRONMENT (V48 - ENTROPY FLOOR + STALE PENALTY + GREEN BONUS)
# ============================================================
...
class MarketMakingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self._is_in_play(current_row):
            ...
            # V48: Green episode bonus — extra reward for profitable episodes
        ...
        step_reward = mtm_reward + sharpe_reward + activity_reward + stale_penalty_this_step
...
# ============================================================
# TRAINING METRICS CALLBACK (V48 — RICH LOGGING + SAC INTERNALS + STALE PENALTY)
# ============================================================

class TrainingMetricsCallback(BaseCallback):
    """Track rich per-episode metrics including violations, commission, SAC internals, and stale penalty."""
    ...
    def _on_step(self) -> bool:
        ...
        if self.episode_count % 10 == 0:
            ...
            print(f"   V48: stale_penalty={avg_stale_pen:.3f}")
...
print(f"  Stale-market penalty: {STALE_TRADE_PENALTY} per attempted stale trade")
print(f"  Green episode bonus: +{GREEN_EPISODE_BONUS} for profitable episodes")
...
# ---------------------------------------------------------------------------
cell_4_src = r'''### CELL 5 - SETUP TRAINING + CHECKPOINT RESUME (V48) ###
...
if checkpoint_path:
    ...
    # V48: Load as ClippedTDTargetSAC with custom_objects so SB3 uses our subclass
    model = ClippedTDTargetSAC.load(checkpoint_path, env=train_env)
    # Attach V48 parameters for the patch
    ...
    # V48: Use ClippedTDTargetSAC with entropy floor
    ...
    apply_v48_patches(model)
    TRAINING_STEPS = 1000000  # V48: 1M steps
    RESUME_MODE = False

print("\n" + "=" * 60)
print("  V48 Setup complete — ready for training!")
print("  (Entropy Floor + Stale Penalty + Green Bonus)")
print("=" * 60)
'''
...
print("=" * 60)
if RESUME_MODE:
    print(f"  CONTINUING V48 training from checkpoint")
    print(f"  Training for {TRAINING_STEPS:,} additional steps")
else:
    print(f"  Starting V48 training run (Entropy Floor + Stale Penalty + Green Bonus)")
