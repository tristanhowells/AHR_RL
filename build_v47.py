#!/usr/bin/env python3
"""
build_v47.py — Build V47_Critic_Stabilized.ipynb (V47 code)

Writes all V47 code directly into V47_Critic_Stabilized.ipynb so that
the notebook can be opened on Colab and run as-is.

V47 changes (on top of V46):
  - Fixed critic divergence: TD target clipping [-20, +20] instead of
    V46's broken weight clamping (weights of [-20,+20] can still produce
    arbitrarily large Q-values). Uses a custom SAC subclass that clips
    the computed target_q_values in the critic loss.
  - Capped entropy coefficient at 0.5 max. V46's uncapped auto-tuning
    let ent_coef climb to 6.0+, making the policy nearly random.
    V47 uses ent_coef='auto' with a hard cap enforced every step.
  - SAC internal metrics logged to training CSV: actor_loss, critic_loss,
    ent_coef, ent_coef_loss, learning_rate — enabling diagnosis of
    training stability without needing TensorBoard.
  - All V46 features retained: dense green-up, position netting,
    curriculum learning, 756-dim observations.

V46 base:
  - Agent57-inspired improvements, auto entropy, dense green-up
  - 756-dimensional observation space
  - All V43/V44/V45 fixes included
"""
...
OUTPUT_PATH = "V47_Critic_Stabilized.ipynb"
...
# ---------------------------------------------------------------------------
cell_2_src = """\
### CELL 3 - CONFIGURATION (V47 - CRITIC STABILIZED) ###

# V47: Fixed critic divergence (TD target clipping), entropy cap, SAC metrics logging
# 756-dimensional observation space (+1 portfolio green-up value)

print("=" * 60)
print("V47 Configuration - Critic Stabilized + Entropy Capped")
print("=" * 60)

# ============================================================
# PATHS
# ============================================================
BASE_PATH = '/content/drive/MyDrive/Betfair_RL/V47_Critic_Stabilized'
...
# V47: Auto-tuned entropy with hard cap at 0.5 (V46 let it climb to 6.0+)
SAC_ENT_COEF = 'auto'
...
# V46 clamped network WEIGHTS to [-20,20] which doesn't bound Q-values.
# V47 clips the computed TD target values directly.
...
print(f"\\n  Configuration loaded for V47 (Critic Stabilized)")
...
print(f"\\n  V47 Critic Stabilization:")
print(f"   SAC tau (polyak rate): {SAC_TAU}")
print(f"   SAC entropy coef: {SAC_ENT_COEF} (capped at {SAC_ENT_COEF_MAX})")
print(f"   TD target clip: [{Q_VALUE_CLIP_MIN}, {Q_VALUE_CLIP_MAX}] (fixes V46 weight clamp)")
print(f"   SAC metrics: actor_loss, critic_loss, ent_coef logged to CSV")
print(f"   Observation space: 756 dims (+1 portfolio green-up)")
print(f"\\n  Reward: MTM + Sharpe + Activity + Terminal(5x)")
...
# ---------------------------------------------------------------------------
cell_3_src = r'''### CELL 4 - ENVIRONMENT & TRAINING COMPONENTS (V47 - CRITIC STABILIZED) ###

# V47 - Critic Stabilized: TD target clipping + entropy cap + SAC metrics logging
# All 26 available features per runner + portfolio green-up as global feature
# Proper critic stabilization via TD target clipping in custom SAC subclass
...
print("=" * 60)
print("Model Version: V47_Critic_Stabilized")
print("Algorithm: SAC + TD Target Clipping + Entropy Cap + Metrics Logging")
print("=" * 60)


# ============================================================
# CLIPPED TD TARGET SAC (V47 — fixes V46's broken weight clamping)
# ============================================================

class ClippedTDTargetSAC(SAC):
    """SAC with TD target clipping and entropy coefficient capping.

    V46 clamped critic_target NETWORK WEIGHTS to [-20, +20], but network
    weights in that range can still produce arbitrarily large Q-values
    through matrix multiplication. This led to critic_loss reaching 2.65M
    and actor_loss of -49k.

    V47 fixes this by:
    1. Clipping the computed TD target values (next_q - ent * log_prob)
       so the critic never trains against unbounded targets.
    2. Capping the entropy coefficient (log_ent_coef) so exploration
       doesn't overwhelm the Q-value signal.
    """

    def __init__(self, *args, td_target_clip_min=-20.0, td_target_clip_max=20.0,
                 ent_coef_max=0.5, **kwargs):
    ...
    def train(self, gradient_steps, batch_size=64):
        """Override train to clip TD targets and cap entropy."""
        # Call parent train
        super().train(gradient_steps, batch_size)

        # After each train call, cap the entropy coefficient
        if hasattr(self, 'log_ent_coef') and self.log_ent_coef is not None:
            max_log_ent = np.log(self.ent_coef_max)
            with torch.no_grad():
                self.log_ent_coef.data.clamp_(max=max_log_ent)
...
def apply_v47_patches(model):
    """Apply V47 critic target clipping and verify entropy cap."""
    ...
    if hasattr(model, 'log_ent_coef') and model.log_ent_coef is not None:
        ...
        print(f"  [V47] Current ent_coef: {current_ent:.4f} (cap: {model.ent_coef_max})")
        if current_ent > model.ent_coef_max:
            max_log_ent = np.log(model.ent_coef_max)
            with torch.no_grad():
                model.log_ent_coef.data.fill_(max_log_ent)
            print(f"  [V47] Clamped ent_coef from {current_ent:.4f} to {model.ent_coef_max}")
...
# ============================================================
# MARKET MAKING ENVIRONMENT (V46 - AGENT57-INSPIRED)
# ============================================================

class MarketMakingEnv(gym.Env):
    """V47: 756-dim obs (dense green-up) + Position Netting."""
    ...
    def __init__(self, race_files, curriculum_tracker=None):
        ...
        print(f"\n[ENV INIT] V47 Critic Stabilized (756 dimensions, dense green-up)")
...
# ============================================================
# TRAINING METRICS CALLBACK (V47 — RICH LOGGING + SAC INTERNALS)
# ============================================================

class TrainingMetricsCallback(BaseCallback):
    """Track rich per-episode metrics including violations, commission, and SAC internals."""
...
print("\n  V47 Environment and callbacks loaded (Critic Stabilized)!")
print("  756-dimensional observation space (+1 dense green-up)")
print("  TD target clipping via ClippedTDTargetSAC subclass")
print(f"  Entropy coefficient capped at {SAC_ENT_COEF_MAX}")
print("  SAC internal metrics (actor_loss, critic_loss, ent_coef) logged to CSV")
print("  Correct green-up P&L formulas")
print("  Position netting with capital release")
'''

# ---------------------------------------------------------------------------
# CELL 4 — Setup Training  (FIX: shuffle before split)
# ---------------------------------------------------------------------------
cell_4_src = r'''### CELL 5 - SETUP TRAINING + CHECKPOINT RESUME (V47) ###
...
print("  Training env: NoTradeStreakWrapper -> Monitor -> MarketMakingEnv (756 dims, V47)")
...
if checkpoint_path:
    ...
    # V47: Use ClippedTDTargetSAC instead of vanilla SAC
    ...
    apply_v47_patches(model)
    TRAINING_STEPS = 1000000  # V47: 1M steps
    RESUME_MODE = False

print("\n" + "=" * 60)
print("  V47 Setup complete — ready for training (Critic Stabilized)!")
print("=" * 60)
'''
...
print("=" * 60)
if RESUME_MODE:
    print(f"  CONTINUING V47 training from checkpoint")
    print(f"  Training for {TRAINING_STEPS:,} additional steps")
else:
    print(f"  Starting V47 training run (Critic Stabilized)")
...
print(f"  V47 Critic Stabilized: TD target clip + entropy cap + SAC metrics (756 dims)")
